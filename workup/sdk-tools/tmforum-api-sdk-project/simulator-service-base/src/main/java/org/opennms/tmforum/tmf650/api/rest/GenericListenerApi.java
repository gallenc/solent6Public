...
public class GenericListenerApi {
    ...
    @Consumes({ "application/json;charset=utf-8" })
    @Produces({ "application/json;charset=utf-8" })
    @io.swagger.annotations.ApiOperation(value = "Client listener for generic events", notes = "NOTE This is not part of the current spec but it defines a listener to listen for any events (GenericEvents) and not just specific event types", response = GenericEventSubscription.class, tags = {
            "notification listeners (client side)", })
    @io.swagger.annotations.ApiResponses(value = {
            @io.swagger.annotations.ApiResponse(code = 201, message = "Notified", response = GenericEventSubscription.class),
